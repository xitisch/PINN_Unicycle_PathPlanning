...
# Obstacle 1
x_c1, y_c1, r1 = 0.4, 0.15, 0.15

# Obstacle 2
x_c2, y_c2, r2 = 0.7, -0.15, 0.15
...
ax_traj = axs[0,0]
axs[0,1].remove()                 # remove top-right axis
ax_traj = fig.add_subplot(2,1,1)  # top plot spanning both columns


# Trajectory plot
ax_traj.plot(x.cpu().numpy(), y.cpu().numpy(), linewidth=2)
ax_traj.scatter([x0, xT], [y0, yT])
...
ax_traj.add_patch(circle1)
ax_traj.add_patch(circle2)
ax_traj.legend()


# Velocity
axs[1,0].plot(t_list.detach().numpy(), v.cpu().numpy())
...
# Angular velocity
axs[1,1].plot(t_list.detach().numpy(), omega.cpu().numpy())
