...
x_c, y_c, w, h = 0.3, 0.1, np.sqrt(2)*0.2, np.sqrt(2)*0.2
obs = [[x_c, y_c, w, h]]
...
all_x = np.concatenate(
    ...
    [np.array([x_c - w/2, x_c + w/2])]
)
all_y = np.concatenate(
    [y for row in results for _, y, _ in row] +
    [np.array([y_c - h/2, y_c + h/2])]
)
...
for row_idx, ((param_name, values), row_results) in enumerate(zip(SWEEPS, results)):
    for col_idx, (x_np, y_np, val) in enumerate(row_results):
        ...
        # Annotation: bottom-left, no bbox — matching longitudinal style
        ax.text(
            0.02, 0.06,
            f"c=({x_c:.2f},{y_c:.2f})\nw={w:.2f}, h={h:.2f}",
...
#  Save
# ------------------------------------------------------------------ #
output_folder = os.path.join("results", "loss_weight", "rectangle")
