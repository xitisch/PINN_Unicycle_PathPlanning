...
# Saves:
#   figures_fixed_r/curvature_vs_xc.png
#   figures_fixed_r/trajectories_grid_1xN.png
#   figures_fixed_r/scenario_results.csv
#   figures_fixed_r/scenario_trajectories.npz
...
from PINNs_functions import *          # hard_bc_transform, derivative, device
from training_NN import train_model    # trainer
...
# Choose ONE of the two options below for y_c:
# OPTION A (recommended): keep the same intrusion Delta relative to the straight line y=0
Delta = 0.10
y_c = r_fixed - Delta
if y_c < 0:
    raise ValueError(f"Delta={Delta} too large for r_fixed={r_fixed}: y_c would be negative.")

# OPTION B: explicitly set transverse offset instead (comment OPTION A and uncomment below)
# y_c = 0.05

# Vary longitudinal obstacle position
x_positions = [0.20, 0.30, 0.40, 0.50, 0.60]
