...
for x_c in x_positions:
    ...
    t_np, kappa_np, kappa_max = compute_curvature(model, t_list=t_train, T=T, BC=BC)
...
# Create lookup by x_c (unique in this experiment)
lookup = {s["x_c"]: s for s in scenarios}

# Precompute kappa curves for consistent y-limits in row 2
kappa_curves = {}
k_all = []
for x_c in x_positions:
    s = lookup[x_c]
    # If you didn't store the model, you need to recompute inside the training loop and store it:
    # scenarios.append({"model": model, ...})
    # So here we assume you've stored it as s["model"].
    t_np, k_np, max_curvature = compute_curvature(s["model"], t_plot, T=T, BC=BC)
    kappa_curves[x_c] = (t_np, k_np)
    k_all.append(np.abs(k_np))

k_all = np.concatenate(k_all)
k_ylim = (0.0, k_all.max() * 1.10)
...
fig.suptitle(
    rf"Trajectories (top) and curvature over time (bottom) — fixed r={r_fixed:.2f}",
    fontsize=14
)
fig.tight_layout(rect=[0, 0, 1, 0.93])

path = os.path.join(output_folder, "trajectories_and_kappa_2xN.png")
