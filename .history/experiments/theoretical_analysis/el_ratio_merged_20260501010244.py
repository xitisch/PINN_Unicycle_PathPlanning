...
def get_effective_weights(model, t_list, T, BC, obs, obstacle_type,
                          lambda_phy, lambda_obs, lambda_v, lambda_omega):
    """
    Replicates the epoch-0 normalization in train_pinn.py to recover
    the effective weights actually used during training.
    """
    with torch.no_grad():
        # Physics loss at epoch 0
        scale_phy = phyics_loss(model, t_list, T, BC).detach()

        # Obstacle loss at epoch 0
        scale_obs = torch.tensor(0.0)
        if obstacle_type == "circle":
            scale_obs = circ_obs_loss(
                model, t_list, obs, T, BC
            ).detach()
        else:
        ...
        # Smoothness losses at epoch 0
        scale_v     = v_loss(model, t_list, T, BC).detach()
        scale_omega = omega_loss(model, t_list, T, BC).detach()

    eps = 1e-8
    lam_phy_eff   = lambda_phy   / (scale_phy   + eps)
    lam_obs_eff   = lambda_obs   / (scale_obs   + eps)
    lam_v_eff     = lambda_v     / (scale_v     + eps)
    lam_omega_eff = lambda_omega / (scale_omega + eps)

    return (lam_phy_eff.item(), lam_obs_eff.item(),
            lam_v_eff.item(),   lam_omega_eff.item())
