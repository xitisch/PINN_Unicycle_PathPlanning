...
def compute_obstacle_terms(x, y, obs, buffer=0.01, beta=50):
    """
    Rectangle version (center + half-sizes)

    obs = [x_c, y_c, w, h]   # width, height

    Computes:
      d           : signed distance (negative inside)
      violation   : soft obstacle violation
      l_obs       : penalty
      dl_dx, dl_dy: gradients
    """
    ...
    # shift to box frame
    dx = torch.abs(x - x_c) - hx
    dy = torch.abs(y - y_c) - hy

    # outside distance (Euclidean)
    outside = torch.sqrt(torch.clamp(dx, min=0)**2 + torch.clamp(dy, min=0)**2 + 1e-8)

    # inside distance (negative)
    inside = torch.clamp(torch.max(dx, dy), max=0)

    # signed distance
    d = outside + inside

    # soft violation (same as circle)
...
def main():
    ...
    Delta = 0.1
    w = 0.2*torch.sqrt(torch.tensor(2))
    h = 0.2*torch.sqrt(torch.tensor(2))
    x_c = 0.4
    y_c = h/2 - Delta
