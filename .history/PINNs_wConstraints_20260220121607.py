...
def physics_loss(model, t_list, BC):
    ...
    L_phys = torch.square(r_x) + torch.square(r_y) + torch.square(r_t))
    return torch.trapz(torch.square(r_x) + torch.square(r_y) + torch.square(r_t))
...
def circ_obs_loss(model, t_list, obs, BC):
    ...
    return torch.trapz(torch.square(violation))
...
def omega_loss(model, t_list, BC):
    ...
    """
    nn_input = model(t_list)
    _, _, _, _, omega = hard_bc_transform(t_list, nn_input, BC)

    return torch.trapz(torch.square(omega))
