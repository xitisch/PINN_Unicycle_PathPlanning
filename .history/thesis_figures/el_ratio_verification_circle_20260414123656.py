...
# =========================
# MASK (avoid division noise)
# =========================
mask_x = np.abs(rhs_x.detach().numpy().flatten()) > 1e-4
mask_y = np.abs(rhs_y.detach().numpy().flatten()) > 1e-4
mask_theta = np.abs(omega.detach().numpy().flatten()) > 1e-4


# =========================
# PLOT 1: Spatial EL ratio
# =========================
plt.figure()

plt.plot(t_np[mask_x], ratio_x_np[mask_x], label="x-direction")
plt.plot(t_np[mask_y], ratio_y_np[mask_y], label="y-direction")

plt.axhline(1.0, linestyle="--", label="Expected = 1")

plt.xlabel("t")
plt.ylabel("Ratio")
plt.title("Spatial EL Verification")
plt.legend()
plt.grid()
plt.show()
...
# =========================
# PLOT 2: Spatial EL error
# =========================
plt.figure()

plt.plot(t_np, error_x_np, label="Error x")
plt.plot(t_np, error_y_np, label="Error y")

plt.axhline(0.0, linestyle="--")

plt.xlabel("t")
plt.ylabel("Error")
plt.title("Spatial EL Residual Error")
plt.legend()
plt.grid()
plt.show()
...
# =========================
# PLOT 3: Angular relation
# =========================
plt.figure()

plt.plot(t_np[mask_theta], ratio_theta_np[mask_theta], label="r_theta / omega")

expected = lambda_smooth / lambda_phy
plt.axhline(expected, linestyle="--", label="Expected ratio")
