...
def phyics_loss(model, t_list, T, BC):
    ...
    r_theta = 4 * (theta_t - omega) # !!!

    sx = r_x.detach().abs().mean() + 1e-8
    sy = r_y.detach().abs().mean() + 1e-8
    st = r_theta.detach().abs().mean() + 1e-8

    r_x_n = r_x / sx
    r_y_n = r_y / sy
    r_theta_n = r_theta / st

    L_phy = (r_x_n**2) + (r_y_n**2) + (r_theta_n**2)
