# Parameter sensitivity: lambda_phy and lambda_obs.
# 4x3 grid:
#   rows 1-2 : circle    (lambda_phy, lambda_obs)
#   rows 3-4 : rectangle (lambda_phy, lambda_obs)
...
def draw_panel(ax, x_np, y_np, obstacle_patch_fn, cx, cy, annotation,
    ...
    ax.plot(cx, cy, marker="x", markersize=7, mew=2)
...
for row_idx, ((param_name, values), row_results) in enumerate(zip(SWEEPS, results_circ)):
    for col_idx, (x_np, y_np, val) in enumerate(row_results):
        ...
        def circ_patches():
            fill = patches.Circle((x_c_circ, y_c_circ), r_obs, fill=True, alpha=0.15, linewidth=0)
            edge = patches.Circle((x_c_circ, y_c_circ), r_obs, fill=False, linewidth=2.5)
            return fill, edge
...
for row_idx, ((param_name, values), row_results) in enumerate(zip(SWEEPS, results_rect)):
    ...
    for col_idx, (x_np, y_np, val) in enumerate(row_results):
        ...
        def rect_patches():
            fill = patches.Rectangle((xmin, ymin), w_rect, h_rect, fill=True, alpha=0.15, linewidth=0)
            edge = patches.Rectangle((xmin, ymin), w_rect, h_rect, fill=False, linewidth=2.5)
            return fill, edge
...
row_labels = [
    r"Circle: $\lambda_{\mathrm{phy}}$",
    r"Circle: $\lambda_{\mathrm{obs}}$",
    r"Rect: $\lambda_{\mathrm{phy}}$",
    r"Rect: $\lambda_{\mathrm{obs}}$",
]
...
fig.suptitle(
    r"Trajectory sensitivity: $\lambda_{\mathrm{phy}}$ and $\lambda_{\mathrm{obs}}$ "
