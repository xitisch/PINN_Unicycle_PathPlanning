...
def plot_dual_axis(ax, t_np, lhs, rhs, lhs_label, rhs_label, title):
    ...
    ax.set_xlabel("$t$")
    ax.set_ylabel(f"LHS: {lhs_label}",  color="tab:blue", fontsize=12)
    ax2.set_ylabel(f"RHS: {rhs_label}", color="tab:red",  fontsize=12)
...
# ============================================================
    ...
    plt.savefig(
        os.path.join(output_folder, "EL_5x2.png"),
        dpi=300
    )
    plt.show()
    plt.close()
    ...
    for row, (data, obs_type, obs_def) in enumerate([
        ...
        if obs_type == "circle":
            ...
            ax.set_title("Trajectory (Circle)")
            ...
            ax.set_title("Trajectory (Rectangle)")

        ax.scatter(BC[0], BC[1], s=60, color="orange")
        ax.scatter(BC[2], BC[3], s=60, color="green")
    ...
    plt.savefig(
        os.path.join(output_folder, "EL_2x5.png"),
        dpi=300
    )
